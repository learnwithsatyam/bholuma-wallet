...
function SendSol({
  ...
  setRecieveAddress: (address: string) => void;
  handleSendSol: () => void;
}) {
  return (
      ...
      <DrawerContent>
        ...
        <div className="space-y-4 items-center space-x-1 px-3">
          <Input
            type="text"
            placeholder="0"
            className="text-6xl md:text-6xl h-fit md:h-fit border-none outline-none text-center dark:focus:outline-none dark:focus:ring-0"
            inputMode="numeric"
            pattern="[0-9]*"
            onChange={(e) => {
        ...
        <DrawerFooter>
          <div className="flex space-x-2">
            <BholumaButton className="w-1/2" onClick={handleSendSol}>
              Send
            </BholumaButton>
